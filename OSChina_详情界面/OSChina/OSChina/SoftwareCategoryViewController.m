...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    SoftwareCategory* category = self.objects[indexPath.row];
        if (_tag == 0) {
            SoftwareCategoryViewController* vc = [[SoftwareCategoryViewController alloc] initWithTag:category.tag];
            [self.navigationController pushViewController:vc animated:YES];
        }else{
            SoftwareViewController* vc = [[SoftwareViewController alloc] initWIthSearchTag:category.tag];
